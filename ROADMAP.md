...
Where fleet is headed. fleet is one bridge over many machines. It ships your repo
...
- [x] Three layers. The control plane `fleet`, the on-host executor `fleet exec`,
  and typed clients that wrap `qsub`, `sbatch`, `pueue`, and `rsync`.
- [x] Onboarding that earns a place in the fleet. `fleet setup` probes the host,
  rsyncs the repo, and runs `chefe install`, so only machines that build the env
  are cached as targets.
- [x] Dispatch and routing. `fleet submit <target> <script>`, plus `submit auto`
...
- [x] Tracking and pull-back. `ps`, `status`, `reconcile`, `logs`, `info`,
  ...
  TinyDB `.fleet/db.json`.
- [x] Targets from `~/.ssh/config`, config from a tiny `fleet.toml`, and a job
  script that runs anywhere with no code changes.

## v0.1.0

Make fleet complete and trustworthy across every host it touches.
...
- [ ] **Friendlier failures.** Clear messages when a host is unreachable, an env
  fails to build, or a scheduler is missing, plus a `fleet doctor` that checks a
  target and reports what is off.
- [ ] **Richer `fleet ls`.** Show drift between cached capabilities and the live
...
Freeze the surface and make fleet safe to depend on.

- [ ] **Stable config and command surface.** A versioned `fleet.toml` schema for
...
- [ ] **Full tested parity** across pueue, SLURM, PBS, LSF, and Kubernetes, on
  every host kind fleet advertises.
- [ ] **A web dashboard.** A read-only view of runs across the whole fleet, live
  state and logs, on top of the same `.fleet/db.json` and `reconcile` data.
- [ ] **Robust self-management.** A `fleet self update`, a hardened installer,
  and signed releases.
...
- [ ] Cost and queue awareness in `submit auto`, routing by price and wait time,
  not just by VRAM.
- [ ] Shared run history across collaborators, so a team sees one fleet.
- [ ] An editor extension on top of the published `fleet.toml` schema.
